...
- 게임 카드의 `스크린샷` 버튼이 실제 다중 사진 선택기를 바로 실행
- 삼성 갤러리 선택이 어려운 경우를 위한 시스템 파일 이미지 선택 경로 추가
- 촬영 파일이 비어 있거나 이미지 가져오기에 실패하면 크래시 대신 안내 표시
- `adjustResize`와 채팅 패널 단위 IME 인셋 적용으로 키보드가 입력창을 가리지 않음
- 키보드 표시 중 진행 요약·힌트 영역을 접고 입력 글자·커서·배경색을 명시적으로 고정
